...
@Version   : 1.0 (Clustering Baseline)
"""
...
def draw_tsne(dataset_name, output_, labels, adj=None, epoch_idx=0, k_value=0, style=""):
    """TSNE visualization of output_ colored by labels."""
    import torch as _torch

    if isinstance(output_, _torch.Tensor):
        x_data = output_.cpu().detach().numpy()
    else:
        x_data = output_
    if isinstance(labels, _torch.Tensor):
        label_data = labels.cpu().detach().numpy()
    else:
        label_data = labels
    ...
    x_tsne = TSNE(n_components=2, learning_rate=100, random_state=42).fit_transform(x_data)
    plt.figure(figsize=(8, 6))

    if adj is not None:
        try:
            if isinstance(adj, list) and hasattr(adj[0], "cpu"):
                adj_np = adj[0].cpu().numpy()
            elif hasattr(adj, "cpu"):
                adj_np = adj.cpu().numpy()
            else:
                adj_np = adj
            for src in range(adj_np.shape[0]):
                for tgt in range(adj_np.shape[1]):
                    if adj_np[src][tgt] > 0:
                        plt.plot(
                            [x_tsne[src, 0], x_tsne[tgt, 0]],
                            [x_tsne[src, 1], x_tsne[tgt, 1]],
                            color="silver",
                            linewidth=0.1,
                            zorder=1,
                        )
    ...
    scatter = plt.scatter(x_tsne[:, 0], x_tsne[:, 1], c=label_data, s=8, cmap="tab10")
    handles, _ = scatter.legend_elements(prop="colors")
    labels_legend = [str(i) for i in range(1, 11)]
    plt.legend(handles, labels_legend, loc="upper right")
    plt.tight_layout()

    out_svg = f"./results/{dataset_name}_tsne_{style}_{epoch_idx}_k{k_value}.svg"
    out_png = f"./results/{dataset_name}_tsne_{style}_{epoch_idx}_k{k_value}.png"
    plt.savefig(out_svg, format="svg", transparent=True)
    plt.savefig(out_png, format="png", dpi=150)
...
def clustering(
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, factor=0.3, patience=15, min_lr=1e-8
    )
    start = time.time()
    acc_max = 0.0
    ...
    with tqdm(total=args.epoch, desc="Training") as pbar:
        for epoch_idx in range(args.epoch):
            ...
            loss_feature_relation_recon = torch.tensor([0.0], device=device)
            loss_graph_topology_recon = torch.tensor([0.0], device=device)
            loss_consis = torch.tensor([0.0], device=device)
            ...
            for view_idx in range(n_view):
                for block_idx in range(args.block):
                    loss_feature_relation_recon += criterion(
                        z_list[block_idx].mm(z_list[block_idx].t()),
                        feature_list[view_idx].mm(feature_list[view_idx].t()),
                    )
                    loss_graph_topology_recon += criterion(
                        z_list[block_idx].mm(z_list[block_idx].t()),
                        adj_list[view_idx],
                    )
            ...
            acc, nmi, purity, ari, fscore, precision, recall = StatisticClustering(
                output_np, labels, n_classes
            )
    ...
    pred_labels_np = np.argmax(output_np, axis=1)
    try:
        draw_tsne(
            dataset_name,
            output_np,
            ...
            epoch_idx=0,
            k_value=args.block,
            style=args.fusion_type,
        )
    except Exception as exc:
        print("TSNE visualization skipped due to:", exc)
    ...
    with open(filename, "a") as f:
        f.write(
            ...
            + str(args.beta)
            + ",lambda_feature_relation = "
            + str(lambda_feature_relation)
            + ",lambda_graph_topology = "
            + str(lambda_graph_topology)
            + ",lambda_consistency = "
...
if __name__ == "__main__":
    ...
    for dataset_id in selected_dataset_ids:
        ...
        print(
            "samples:{}, view size:{}, feature dimensions:{}, class:{}".format(
                n, n_view, n_feats, n_classes
            )
        )
