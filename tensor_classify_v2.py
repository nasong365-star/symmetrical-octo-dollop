...
def classifier(train_indices, test_indices, num_samples, feature_list, sim_list, lap_list, lap2_list, labels_np,
    ...
    # - Default mode uses standard training (`loss_sup` + entropy regularization).
    # - Ablation toggles are kept below for quick experiments.
    ...
    for repeat_idx in range(num_repeats):
        ...
        with tqdm(total=args.epoch, desc="Training") as pbar:
            for epoch in range(args.epoch):
                ...
                # Default: supervised loss is enabled.
                # Ablation option A (remove supervised signal):
                # loss_sup = torch.Tensor(np.array([0])).to(device)
                loss_sup = loss_function(F.log_softmax(model_outputs[-1], dim=1)[train_indices],
                                         labels_tensor[train_indices])

                # Default: entropy regularization is enabled.
                # Ablation option B (remove consistency term):
...
if __name__ == '__main__':
    ...
    for dataset_id in selected_dataset_ids:
        ...
        # Move all per-view tensors to the selected device.
